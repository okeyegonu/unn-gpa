#!/usr/bin/env node
/**
 * Builds dist/unn-gpa-calculator.html — the whole calculator as ONE file.
 *
...
const MODULES = ['src/grading.js', 'src/catalogue.js', 'src/gpa-engine.js', 'src/storage.js', 'src/ui.js'];
...
out = out.replace(
  '<title>',
  `<!--\n  UNN GPA Calculator — single-file build.\n` +
  `  Built ${new Date().toISOString()} by tools/build-single-file.mjs.\n` +
  `  Open this file in Firefox or Chrome. Nothing is sent anywhere; your courses and\n` +
  `  grades stay in your own browser.\n-->\n<title>`,
);

// The bundle must not load anything external.
...
const target = resolve(root, 'dist/unn-gpa-calculator.html');
await writeFile(target, out);

const kb = (Buffer.byteLength(out) / 1024).toFixed(1);
console.log(`Wrote dist/unn-gpa-calculator.html  (${kb} kB)`);
