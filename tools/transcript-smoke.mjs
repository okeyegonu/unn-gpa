...
/** Add a course through the form, as a student would. */
const addCourse = async (c) => {
  await exec(`
    ...
    document.getElementById('f-department').value = 'Physics';
...
/** Grade every listed course of the given years. */
const gradeYears = (years, grade) => exec(`
  ...
  Array.from(document.querySelectorAll('tr[data-id]')).forEach(function (tr) {
    var section = tr.closest('.semester');
    var heading = section ? section.querySelector('h3').textContent : '';
    var match = /^(First|Second|Third|Fourth|Fifth|Sixth|Seventh|Eighth) year/.exec(heading);
    var names = ['First','Second','Third','Fourth','Fifth','Sixth','Seventh','Eighth'];
    var year = match ? names.indexOf(match[1]) + 1 : 0;
...
try {
  ...
  // This calculator has no curriculum: the student lists their own courses.
  for (const c of [
    { code: 'PHY 101', title: 'General Physics I', units: 3, year: 1, semester: 1 },
    { code: 'MTH 111', title: 'Elementary Mathematics I', units: 3, year: 1, semester: 1 },
    { code: 'CHM 101', title: 'Basic Chemistry', units: 2, year: 1, semester: 2 },
    { code: 'PHY 201', title: 'Thermal Physics', units: 3, year: 2, semester: 1 },
    { code: 'PHY 203', title: 'Optics', units: 3, year: 2, semester: 1 },
    { code: 'PHY 202', title: 'Electromagnetism', units: 3, year: 2, semester: 2 },
    { code: 'PHY 301', title: 'Quantum Mechanics', units: 3, year: 3, semester: 1 },
  ]) await addCourse(c);
  check('the student\'s own courses are listed', (await exec(`return document.querySelectorAll('tr[data-id]').length;`)) === 7);
  ...
  check('year one graded', gradedY1 === 3, `${gradedY1} courses`);
  ...
  const verbatim = await exec(`
    return Array.from(document.querySelectorAll('#print-root table.results tr')).map(function (tr) {
      return { code: tr.children[0].textContent.trim(), title: tr.children[1].textContent.trim() };
    });
  `);
  check('the course code and title appear exactly as the student wrote them',
    verbatim.some((r) => r.code === 'MTH 111' && r.title === 'Elementary Mathematics I') &&
    verbatim.every((r) => r.title !== r.title.toUpperCase() || !/[a-z]/.test(r.title)),
  ...
  const stored = await exec(`
    var prefs = JSON.parse(localStorage.getItem('unn-gpa-calculator:prefs') || '{}');
    ...
    return {
      ...
      resultKeys: Object.keys(rec).sort().join(',')
    };
  `);
  ...
  check('and no personal detail reaches the results record',
    stored.resultKeys === 'courses,grades,profile,repeats,saved_at,schema_version',
    stored.resultKeys);
